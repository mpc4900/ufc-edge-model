...
def _compat_position_sizing(
    probability, price, bankroll, experience, cost_buffer=0.02,
    kelly_fraction=0.25, max_position_pct=0.05, reliability_fights=8,
):
    """Keep rolling Streamlit deploys alive while model_engine.py catches up."""
    probability = engine.safe_float(probability)
    price = engine.safe_float(price)
    bankroll = max(0.0, engine.safe_float(bankroll, 0.0))
    experience = max(0.0, engine.safe_float(experience, 0.0))
    if not (0 < probability < 1 and 0 < price < 1):
        return {
            "effective_entry": np.nan, "gross_edge": np.nan, "net_edge": np.nan,
            "full_kelly": 0.0, "kelly_fraction": kelly_fraction,
            "data_reliability": 0.0, "uncapped_position_fraction": 0.0,
            "position_fraction": 0.0, "position_dollars": 0.0,
            "max_position_pct": max_position_pct, "expected_profit": np.nan,
        }
    ...
    reliability = min(1.0, experience / max(1.0, reliability_fights))
    uncapped = full_kelly * kelly_fraction * reliability
    fraction = min(max_position_pct, uncapped)
    position = round(bankroll * fraction, 2)
    return {
        "effective_entry": effective_entry, "gross_edge": gross_edge,
        "net_edge": net_edge, "full_kelly": full_kelly,
        "kelly_fraction": kelly_fraction, "data_reliability": reliability,
        "uncapped_position_fraction": uncapped, "position_fraction": fraction,
        "position_dollars": position, "max_position_pct": max_position_pct,
        "expected_profit": position * (probability / effective_entry - 1),
    }


def _compat_entry_timing(
    probability, price, cost_buffer=0.02, min_edge=0.03,
    experience=0, min_prior_fights=3, start_utc="",
):
    probability = engine.safe_float(probability)
    price = engine.safe_float(price)
    max_entry = probability - cost_buffer - min_edge if np.isfinite(probability) else np.nan
    start = pd.to_datetime(start_utc, utc=True, errors="coerce")
    hours = (start.to_pydatetime() - datetime.now(timezone.utc)).total_seconds() / 3600 if pd.notna(start) else np.nan
    if not 0 < price < 1:
        signal, reason = "WAIT FOR LIVE PRICE", "No executable ask is available."
    elif experience < min_prior_fights:
        signal, reason = "PASS — THIN DATA", f"Only {int(experience)} prior UFC fights on the lower-experience side."
    elif price <= max_entry:
        signal, reason = "ENTER NOW", f"Ask {price:.1%} is at or below the {max_entry:.1%} maximum entry."
    else:
        signal, reason = "WAIT", f"Buy only at {max_entry:.1%} or lower; current ask is {price:.1%}."
    if np.isfinite(hours):
        reason += f" {max(0.0, hours):.0f} hours to scheduled start."
    return {"timing_signal": signal, "timing_reason": reason, "max_entry_price": max_entry, "hours_to_event": hours}
...
binary_position_sizing = getattr(engine, "binary_position_sizing", _compat_position_sizing)
...
entry_timing = getattr(engine, "entry_timing", _compat_entry_timing)
