...
def event_market_history(event_name):
    """Return only the price snapshots that belong to the event on screen."""
    ...
    try:
        ...
        return pd.DataFrame()
    if history.empty or "event" not in history.columns:
        return history
    target = canonical_name(event_name)
    return history[history["event"].map(canonical_name) == target].copy()


def report_filename(event_name):
    """Create a stable, audit-friendly filename for the current screen snapshot."""
    event_slug = re.sub(r"[^A-Za-z0-9]+", "_", str(event_name or "UFC_Event")).strip("_")
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%SZ")
    return f"UFC_Edge_{event_slug}_{timestamp}.xlsx"


def website_excel_report(analyses, event_name, prediction_log, bankroll, cost_buffer,
                         min_edge, min_fights, max_card_exposure, market_history=None):
    """Build the report from the exact model and market snapshot displayed in the app."""
    return build_excel(
        analyses,
        bundle,
        prediction_log,
        ...
        min_edge=min_edge,
        min_prior_fights=int(min_fights),
...
def live_event_board(option, bankroll, min_edge, cost_buffer, min_fights, max_card_exposure, manual_odds, research_texts):
    ...
    try:
        ...
        history = update_market_history(STATE_DIR, current)
        ...
        current_log = load_prediction_log(STATE_DIR)
        event_name = current[0].get("event", search) if current else search
        event_history = history
        if isinstance(history, pd.DataFrame) and len(history) and "event" in history.columns:
            target = canonical_name(event_name)
            event_history = history[history["event"].map(canonical_name) == target].copy()
        excel_bytes = website_excel_report(
            current,
            event_name,
            current_log,
            bankroll,
            ...
            min_fights,
            max_card_exposure,
            market_history=event_history,
        )
        st.download_button(
            "Download this event's Excel report",
            excel_bytes,
            file_name=report_filename(event_name),
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key=f"live-excel-{key_suffix}",
            width="stretch",
        )
        if matched == 0:
    ...
    if not menu:
        ...
        if selected["kind"] == "recorded":
            ...
            excel_bytes = website_excel_report(
                ...
                min_fights,
                max_card_exposure,
                market_history=event_market_history(selected_event["event"]),
            )
            st.download_button(
                "Download this event's Excel report",
                excel_bytes,
                file_name=report_filename(selected_event["event"]),
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
        else:
